...
module.exports = {
    APDU: function() {
        ...
        this.IN_BLOCKSIZE = 0x80;
        this.OUT_BLOCKSIZE = 0x100;

        this.STATE_INITIAL = 0;
        this.STATE_PARTIAL_INCOMING = 1;
        this.STATE_FULL_INCOMING = 2; 
        this.STATE_OUTGOING = 3;
        this.STATE_OUTGOING_LENGTH_KNOWN = 4;
        this.STATE_PARTIAL_OUTGOING = 5;
        this.STATE_FULL_OUTGOING = 6;

        this.PROTOCOL_MEDIA_CONTACTLESS_TYPE_A = 0;
        this.PROTOCOL_MEDIA_CONTACTLESS_TYPE_B = 1;
        this.PROTOCOL_MEDIA_DEFAULT = 2;
        this.PROTOCOL_MEDIA_MASK = 3;
        this.PROTOCOL_MEDIA_USB = 4;
        this.PROTOCOL_T0 = 5;
        this.PROTOCOL_T1 = 6;
        this.PROTOCOL_TYPE_MASK = 7;
        this.STATE_ERROR_IO = -1;
    ...
    constr: function (APDU, bArray) {
        ...
        if (bArray.length == 4) { APDU.type = 1; APDU.Nc = 0; APDU.Ne = 0; APDU.lelength = 0; }
            ...
            temp = this.getOutLengths(APDU.buffer, bArray.length);
            APDU.Ne = temp[0]; APDU.lelength = temp[1];

        } else if ((bArray.length == APDU.buffer[4] + 5) || (bArray.length == this.getInLengths(APDU.buffer)[0] + 5)) {
            APDU.type = 3;
            temp = this.getInLengths(APDU.buffer); APDU.Nc = temp[0]; APDU.lclength = temp[1];
            APDU.Ne = 0; APDU.lelength = 0; APDU.cdataoffs += APDU.lclength - 1;
        } else {
            //self.type = 4; //REMOVED BY ADAM
            if(bArray.length > 6) {
                temp = this.getInLengths(APDU.buffer); APDU.Nc = temp[0]; APDU.lclength = temp[1];
                temp = this.getOutLengths(APDU.buffer, bArray.length); APDU.Ne = temp[0]; APDU.lelength = temp[1];
    ...
    setOutgoing: function (APDU) { //07
        ...
        var ogl = this.getOutgoingLength(APDU);
    ...
    setOutgoingAndSend: function (APDU, bOff, len) {
        ...
        this.setOutgoing(APDU);
        this.setOutgoingLength(APDU, len);
        this.sendBytes(APDU, bOff, len);
